...
def encrichment_cell_heatmap(
    ...
    if not any(
        x in data["source"].iloc[0]
        for x in ("GO-TERM", "KEGG", "REACTOME", "specificity")
    ):
        raise ValueError(
            "Invalid value for 'source' in data. Expected: 'GO-TERM', 'KEGG', 'REACTOME' or 'specificity'."
        )
    ...
    stat_col = [
        ...
        if test in x and adj in x and parent_col.upper() not in x.upper()
    ][0]

    if parent_inc and data["source"].iloc[0] != "specificity":
        data[term_col] = data.apply(
            lambda row: f"{row[parent_col]} -> {row[term_col]}", axis=1
        )

    data = data.loc[data.groupby([set_col, term_col])[stat_col].idxmin()].reset_index(
        drop=True
    )

    data = (
        data.sort_values(stat_col, ascending=True).groupby(set_col).head(top_n)
...
def draw_cell_conections(data, top_n=15):
    ...
    nx.spring_layout(G, weight="weight", k=0.1, iterations=500)
